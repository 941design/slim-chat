...
app.on('ready', async () => {
  ...
  registerHandlers({
    ...
    nostling: {
      ...
      markMessagesRead: (identityId, contactId) => getNostlingService().markMessagesRead(identityId, contactId),
      getUnreadCounts: (identityId) => getNostlingService().getUnreadCounts(identityId),
